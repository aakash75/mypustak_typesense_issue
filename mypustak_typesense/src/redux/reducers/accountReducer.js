...
export default function accountReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case GETADDRESS:
      ...
      const all_address = action.payload.output;
      const primary_address = all_address.filter(
        (address) => address.is_primary == "Y"
      );
      let primary_state_address
      if (primary_address.length) {
        primary_state_address = primary_address[0]
      }
      return {
        ...state,
        getadd: action.payload.output,
        primary_address: primary_state_address,
        fetching_address_loader: false,
      };

    case GETADDRESS_PERPAGE:
      // alert("hi")
      // console.log(action.payload.output,"123456")
      const all_address_per_page = action.payload.output;
      const primary_address_per_page = all_address_per_page.filter(
        (address) => address.is_primary == "Y"
      );
      let primary_state_address_per_page
      if (primary_address_per_page.length) {
        primary_state_address_per_page = primary_address_per_page[0]
      }
      return {
        ...state,
        getadd_per_page: action.payload.output,
        primary_address_per_page: primary_state_address_per_page,
        fetching_address_loader: false,
      };
    ...
    case GETSELECTEDADDRESS_PERPAGE:
      // alert('redu')
      return {
        ...state,
        selectedAddress: action.payload,
    ...
    case FETCH_USERS_DETAIL:
      // alert("userDistinct")
      console.log(action.payload, "98811")
      return {
        ...state,
    ...
    case fETCH_COMMUNICATE_PREFERENCE:
      // console.log(action.payload.response[0],"565999")
      return {
        ...state,
