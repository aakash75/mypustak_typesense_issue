...
//   let authorname1 = params.split("-").join(" ").replace("books", "");
//   let authorname = authorname1.split(",");
//   console.log(query.author_name, authorname1.split(","), "query.author_name");
//   console.log(authorname, "authorname...............");

//   // let og_url = "https://www.mypustak.com/author/" + query.author_name;
...
//   // } else {
//   //   title_tag = "Author " + query.author_name + "|used books online India !";
//   //   meta_description =
//   //     "Author " + query.author_name + "Only online free books used bookstore";
//   // }
//   // console.log(title_tag, "||", meta_description);

//   return {
//     // props: { query, authorname, title_tag, meta_description, og_url: og_url },
//   };
// }
...
function Page(props) {
  ...
  // const [orAuthor,setOrAuther]=useState(null)
  const [drawerOpen, SetdrawerOpen] = useState(false);
  // for Routing error
  let authorname1 = props.params.author_name
    .split("-")
    .join(" ")
    .replace("books", "");
  let authorname = authorname1.split(",");
  ...
  const onSearchStateChange = (updatedSearchState) => {
    ...
    debouncedSetStateRef.current = setTimeout(() => {
      ...
      console.log(authorname, "authorvar");
      let authorvar = "";
      let allauthors = "";
      authorname.map((a) => {
        authorvar = a.split(" ").join("-");
        if (allauthors.length > 1) {
          allauthors = allauthors + "," + authorvar;
        } else {
          allauthors = authorvar;
        }
        console.log(allauthors, "authorvar");
      });
      let urlvar =
        "/author/books" +
        allauthors +
  ...
  const routing = {
    router: history({
      createURL({ qsModule, routeState, location }) {
        ...
        console.log(props, "author_name123");

        //   return `/author/${props.query.author_name}${queryString}`
        return `/author/${authorname}${queryString}`;
      },
    ...
    <React.Fragment>
      ...
      <div>
        <InstantSearch
          ...
          routing={routing}
        >
          <Configure filters={`author:[${authorname}]`} />
          ...
          >
            <div
              className={`${styles.filterDiv} bg-white  col-3 d-none d-sm-block d-md-block d-lg-block`}
              style={{
                background: "#fff",
                maxWidth: "14.563rem",
                minWidth: "14.563rem",
