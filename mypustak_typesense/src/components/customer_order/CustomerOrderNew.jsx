...
import {
  ...
  newOrderdetails,
  CancelOrdergid,
  ConvertCODtoPrepaid_gid,
  cancil_request_action
...
class Order extends Component {
  ...
  loadmore_order = async () => {
    ...
    let res = await this.props.newOrderdetails(newpage);
    if (res) {
 ...
 async componentDidMount() {
  ...
  await  this.props
      ...
      .then((res) => {
        ...
        if (res.length) {
          this.props
            .newOrderdetails(this.state.page)
            .then((ress) => {
              this.setState({ dataCommingLoader: false });
            })
            .catch((errr) => {
              console.log(errr, "lll");
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.state.FilterValue !== prevState.FilterValue) {
      ...
      this.props
        .newOrderdetails(this.state.page, null, body)
        .then((ress) => {
          this.setState({ dataCommingLoader: false });
        })
        .catch((errr) => {
  ...
  CancelOrder = (data,index) => {
    ...
    this.setState({
      CancelOrderPopup: true,
      clickedId: data.order_group_id,
      clickIndexid: index,
    });
    this.setState({ cancelOrderId: data.order_group_id });
  };
  ...
  handleCodPay = (order, cod_charge) => {
    this.setState({ disableCodPay: true, clickedId: order.order_group_id });
    const prepaid_order_amount = (order.payable_amount - 50) * 100;
    const SendDataRzpzy = {
      data: {
        ref_id: order.order_group_id,
  ...
  ConfirmCancelOrder = () => {
    ...
    const token = localStorage.getItem("user_info");
    let user_by = JSON.parse(token).email;
    console.log(user_by, "token.....");

    const Data = {
      data: {
        order_id: this.state.cancelOrderId,
        reason: "customer Dont Want book",
        updated_by: user_by,
      },
    };
    // const token = `Token ${this.props.userToken}`;
  ...
  ConfirmCancelPrepaidOrder = () => {
    ...
    // const token = `Token ${this.props.userToken}`
    this.props
      .cancil_request_action(Data)
      .then((res) => {

        let newPage = parseInt(this.state.clickIndexid / 10) + 1;
        this.props
          .newOrderdetails(newPage, this.state.clickIndexid)
          .then((ress) => {
            this.props.enqueueSnackbar("Order Cancel Requested Successfully Submitted.", {
              variant: "Success",
            });
            this.closeDialog()
            this.setState({
  ...
  render() {
    const { newGetorderdetails, LoadingUserOrder, NomoreUserOrderData } =
      this.props;
              ...
              {this.state.isDataAvailble ? (
                <div>
                  {!this.state.dataCommingLoader ? (
                    newGetorderdetails.length ? (
                      <div className={`${styles.order_div}`}>
                        <div style={{ flex: 10 }}>
                          {newGetorderdetails.map((order, index) => {
                            return (
                              <div key={index}>
                                <ColapsibleOrders
                                  data={order}
                                  ...
                                  CancelOrder={() =>this.CancelOrder(order,index)}
                                  CancelprepaidOrder={() =>this.CancelprepaidOrder(order,index)}
...
const mapStateToProps = (state) => ({
  ...
  newGetorderdetails: state.orderdetailsR.newGetorderdetails,
