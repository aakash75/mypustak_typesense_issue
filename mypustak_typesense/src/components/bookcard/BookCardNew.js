...
function BookCard(props) {
  ...
  useEffect(() => {
    props.list_wishlist.map((data) => {
      if (data.book_id == props.book.bookId) {
  ...
  const DicountedPrice = (price) => {
    const discount_per = props.book.discountPercent;
  ...
  const onClick_book_web = () => {
    // alert("Book   " + props.book);
    // console.log(props.book, " props.book");
    // let slug = props.book.title.split(" ").join("-");
    // const url = `${slug}-${props.book.isbn ? props.book.isbn : ""}`;

    let url =
      createSlug(props.book.slug.replaceAll("-", " ")) +
      "-" +
      createSlug(props.book.author.replace(",", " "));
    if (props.book.isbn) {
      url = url + "-" + props.book.isbn;
    }
    url = url.replaceAll("--", "-");
    url = url.replaceAll(",");
    ...
    // }
    window.open(`/product/${encodedUrl}?${props.book.bookId}`);
    // window.location.assign(
    //   `/product/${encodedUrl}?${props.book.bookId}`,
    //   undefined,
    //   { shallow: false }
    // );
  };
    ...
    if (props.userComponentStatus == 2) {
      if (!clicked) {
        ...
        const wish_data = {
          ...
          user_id: user.id,
          book_inv_id: 0,
          book_thumb: props.book.imageUrl,
          i_date: current_date,
          is_move_incart: 0,
          is_deleted: 0,
          qty: 1,
          cashbacked_price: 0,
          discounted_price: 0,
          cashback_per: props.book.cashbackPercent
            ? props.book.cashbackPercent
            : 0,
          discount_per: props.book.cashbackPercent
            ? props.book.cashbackPercent
            : 0,
          offertype: null,
          user_pay: 0,
  ...
  const openCartDialog = async () => {
    ...
    try {
      ...
      let result = await fetch(
        `${url}/api/v1/get/product/v2/new/${props.book.bookId}/${0}`
      );
      ...
      if (Object.keys(response.old_book).length > 0) {
        ...
        enqueueSnackbar("Sorry this book is out of Stock!", {
          variant: "warning",
        });
        setaddtocartloading(false);
      }
      ...
      enqueueSnackbar(
        `something went wrong! Try another book if repeated please contact mypustak support`,
        { variant: "error" }
      );
      setaddtocartloading(false);
    }
  ...
  const CashbackPrice = (price) => {
    let cashbackedPrice = price - (price * props.book.cashbackPercent) / 100;
  ...
  // console.log(props, "props_book");
  return (
    <div>
      <div
        data-pid={props.book.bookId}
        style={{ position: "relative" }}
        className={
          window.screen.width < 500 ? `` : `${styleBookCard.mainUpperBookDiv}`
        }
      >
        <div className={`${styleBookCard.mainBookDiv}`} style={{}}>
          {props.book?.isMostLoved ? (
            <div className={styleBookCard.walletOfferDiv}>
              <span
                className={styleBookCard.Stick2}
                itemProp={"Trending"}
                data-info={"Trending"}
              >
                Most Loved
          ...
          >
            ...
            <MediaQuery minWidth={577}>
              <div
                style={{
                  opacity: props.book.isOutOfStock == "y" ? 0.5 : null,
                  position: "relative",
                }}
                className={`${styleBookCard.bookImage} h-[15rem] `}
              >
                <Image
                ...
                {currentUrlPath == "/" ? null : (
                  ...
                  >
                    {wishListLoader ? (
                      ...
                      <div
                        className={`${styles.heartdiv}   ${clicked == "click_wishlist"
            ...
            <MediaQuery maxWidth={576}>
              <div>
                <div
                  ...
                  className={`${styleBookCard.bookImage}  h-[14rem] `}
                  onClick={() => {
                    onClick_book();
                    // let slug = props.book.title.split(" ").join("-");
            ...
            <div
              style={{ opacity: props.book.isOutOfStock == "y" ? 0.5 : null }}
              ...
              <MediaQuery minWidth={577}>
                <h3
                  onClick={() => {
                    ...
                    // let slug = props.book.title.split(" ").join("-");
                    // const url = `${slug}-${
                    //   props.book.isbn ? props.book.isbn : ""
                    // }`;
                    // const encodedUrl = encodeURIComponent(url);
                    // window.open(`/product/${encodedUrl}?${props.book.bookId}`);
                    // // location.reload();
              ...
              {props.book.author == "na" ||
                ...
                >
                  ...
                  {props.book?.author?.length > 15
                    ? props.book.author.substring(0, 25).concat("...")
                    : props.book.author}
                </span>
              )}
              {props.book.bookType ? (
                props.book.bookType != 0 ? (
                  <span className={`${styleBookCard.prices}`}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "#098041" }}>
                      ({props.book.discountPercent}% Off)
                    </span>
                  </span>
                ) : (
                  <span className={`${styleBookCard.prices}`}>
        ...
        <div style={{ position: "absolute", bottom: "10px", width: "100%" }}>
          <center>
              ...
              {props.book.isOutOfStock == "n" ||
                ...
                <Button
                  ...
                  style={{
                    ...
                    background: incart.includes(props.book.bookId)
                      ? "linear-gradient(90deg, #ff6600 0%, #e05a00 100%)"
                      : "linear-gradient(90deg, #2157ad 0%, #6190da 100%)",
                  }}
                  disabled={addtocartloading || props.book.isOutOfStock == "y"}
                  className={`button ${incart.includes(parseInt(props.book.bookId))
                    ? "alreadyAddedToCart"
                    : ""
                    }`}
                  onClick={
                    addtocartloading
                      ? null
                      : incart.includes(props.book.bookId)
                        ? () => {
                          goToCart();
                        }
                        : openCartDialog
            ...
            <MediaQuery maxWidth={576}>
              {props.book.isOutOfStock == "n" ||
                ...
                <Button
                  ...
                  disabled={props.book.isOutOfStock == "y"}
                  className={`button ${incart.includes(parseInt(props.book.bookId))
                    ? "alreadyAddedToCart"
                    : ""
                    }`}
            ...
            {/* <Button style={{ width: "90%", }} variant="contained" color="primary">hb fhbfhb</Button> */}
          </center>
        </div>
      </div>

      {/* ===================================== Dialog and snackbar code ==========================================  */}
      <MediaQuery minWidth={577}>
        <Dialog
          open={AddedtoCart}
          keepMounted
                ...
                >
                              ...
                              {props.book.discountPercent ? (
                                ...
                                >
                                  <div>
                                    <div>
                                      <span>
                                        Flat {props.book.discountPercent}% off !{" "}
                                      </span>
                                    </div>
                  ...
                  <div>
                    <Button
                      ...
                      fullWidth
                      disabled={
                        Add_cartLoader || props.book.isOutOfStock == "y"
                      }
                      value={Add_cartLoader ? `Adding` : `Add to Cart 1955`}
                      onClick={(e) => {
                        AddNewBookToCart(e);
                      }}
                    >
                      ...
                      {ConditionData.publication == "NA" ||
                        ...
                        <span>
                          ...
                          <Link
                            ...
                            as={`/publication/books-${ConditionData.publication
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: "#555" }}>
                    {MyCart.offertype == "cashback" ? (
                      <span>₹{MyCart.bookPrice}</span>
                    ) : (
                      <s>{MyCart.bookPrice}</s>
                    )}{" "}
                    <span
                    ...
                    >
                      {MyCart.offertype == "cashback" ? (
                        <span style={{ fontSize: "0.78rem" }}>
                          {" "}
                          Effective price ₹{MyCart.cashbackedPrice}
                        </span>
                      ) : (
